...
@Service
public class GetBookByExternalIdDataProvider implements GetBookByExternalIdGateway {
    ...
    @Override
    public Optional<Book> execute(final Long externalId) {
        final EntityGraph entityGraph = EntityGraphUtils.fromAttributePaths("author");
        final Optional<BookEntity> optional = bookRepository.findByExternalId(externalId, entityGraph);
