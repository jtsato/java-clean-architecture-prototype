...
@Transactional
@Service
public class RemoveBookByBKeyDataProvider implements RemoveBookByBKeyGateway {
    ...
    @Override
    public Optional<Book> execute(final Long bKey) {

        final EntityGraph entityGraph = EntityGraphUtils.fromAttributePaths("author");
        final Optional<BookEntity> optional = bookRepository.findByBKey(bKey, entityGraph);
