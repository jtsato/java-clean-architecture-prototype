...
public class FindAuthorsByIdsController implements FindAuthorsByIdsApiMethod {
    ...
    public FindAuthorsByIdsWrapperResponse execute(@RequestBody @DefaultValue final FindAuthorsByIdsRequest request) {

        final String jsonRequest = JsonConverter.of(request);
        log.info("Starting Controller -> FindAuthorsByIdsController with {}", jsonRequest);

        final Page<Author> authors = findAuthorsByIdsUseCase.execute(request.getIds());
